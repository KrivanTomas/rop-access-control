...
bool KuroRFID::read_token_from_uart(HardwareSerial* use_serial, uint8_t* token, uint8_t max_attempts, uint8_t timeout) {
...
bool KuroRFID::void_frag(HardwareSerial* use_serial, bool* do_void, unsigned long* last_read, unsigned long void_duration) {
...
int KuroRFID::hex_to_int(uint8_t a) {
  if(a >= '0' && a <= '9'){
    return a - 48;
  }
  return a - 55;
...
int KuroRFID::hex_pair_to_int(uint8_t a, uint8_t b) {
  return (hex_to_int(a) << 4) + hex_to_int(b);
}

bool KuroRFID::token_checksum(uint8_t *token){
